...
#  5. Forks a background tasks that randomly triggers a server config reload (via kubectl)
#  6. Runs the test (via go run)
#  7. Stops the background task issuing random reloads
...
CLUSTER_SIZE=5
MAX_RELOAD_DELAY=5 # Maximum time between reloads
